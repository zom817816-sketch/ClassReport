...
& $python -m PyInstaller --noconfirm --clean --onefile --name $appName --distpath $distRoot --workpath (Join-Path $stagingRoot 'work') --specpath $stagingRoot --collect-data matplotlib --collect-data reportlab (Join-Path $projectRoot 'teacher_launcher.py')

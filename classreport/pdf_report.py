...
def register_fonts() -> tuple[str, str]:
    """Use Windows SimHei when present; this keeps Chinese text embedded and legible."""
    if "ReportChinese" not in pdfmetrics.getRegisteredFontNames():
        candidates = [Path(r"C:\Windows\Fonts\simhei.ttf")]
...
class PdfReportBuilder:
    ...
    def _cover(self, c: Canvas, a: ReportAnalysis) -> None:
        ...
        c.setFont(FONT, 10.5)
        c.drawCentredString(PAGE_W / 2 + 78, 486, f"报告时间：{self.settings.report_date.isoformat()}")
        c.drawCentredString(PAGE_W / 2 + 70, 467, f"指导老师：{self.settings.teacher}")
    ...
    def _charts_page(self, c: Canvas, a: ReportAnalysis) -> None:
        ...
        c.setFillColor(MUTED)
        c.setFont(FONT, 8.5)
        c.drawString(48, 712, f"本图组展示 {a.student.name} 的个人成绩与 {len(a.student.classmates_consolidation)} 人班级平均的双维度对比。")
    ...
    def _summary_page(self, c: Canvas, a: ReportAnalysis) -> None:
        ...
        c.setFillColor(BLUE)
        c.setFont(FONT_BOLD, 11.5)
        c.drawString(70, advice_y - 16, "复习方法（建议家长参与）")
        methods = [
    ...
    def _note(self, c: Canvas, text: str, y: float) -> None:
        c.setFillColor(LIGHT_BLUE)
        c.roundRect(48, y - 18, 499, 20, 2, fill=1, stroke=0)
        c.setFillColor(MUTED)
        c.setFont(FONT, 8.4)
        c.drawString(55, y - 11, text)
    ...
    def _callout(self, c: Canvas, x: float, y: float, width: float, height: float, lines: list[tuple[Color, str]]) -> None:
        ...
        for color, text in lines:
            c.setFillColor(color)
            self._wrapped(c, text, x + 11, line_y, width - 22, 9.5, 15, color, max_lines=2)
            line_y -= 32
    ...
    def _watermark(self, c: Canvas) -> None:
        c.saveState()
        c.setFillColor(HexColor("#EFF8FC"))
        c.setFont(FONT_BOLD, 30)
        c.translate(PAGE_W / 2, PAGE_H / 2)
        c.rotate(36)
        c.drawCentredString(0, 0, "L2 2光 夏季班 光明顶")
        c.restoreState()
